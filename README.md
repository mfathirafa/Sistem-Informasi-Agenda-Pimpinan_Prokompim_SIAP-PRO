...
- Deploy ke **Vercel** (gratis), database di **Supabase** (gratis)
...
3. Buat akun **Supabase** (database gratis) di https://supabase.com — daftar pakai akun GitHub juga bisa.
...
Buka `.env`, isi tiga hal:
- `DATABASE_URL` — buat database baru di dashboard Supabase, salin "Connection string" (via connection pooler) ke sini.
- `DIRECT_URL` — connection string direct Supabase (tanpa pooler, untuk Prisma Migrate).
...
- Sudah ada log riwayat perubahan data (Activity Log) — siapa mengubah apa, kapan, bisa dilihat di menu Activity Log.
- Sudah ada halaman Laporan SPJ (export XLSX + cetak) dan Kalender Kegiatan bulanan.
